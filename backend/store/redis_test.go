...
func Test_NewRedisKVS(t *testing.T) {
	...
	_, err := NewRedisKVS(ctx, "127.0.0.1", 6379, "default", "redispw", 10)
	if err != nil {
...
func Test_Save(t *testing.T) {
	ctx := context.Background()
	kvs, err := NewRedisKVS(ctx, "127.0.0.1", 6379, "default", "redispw", 10)
	if err != nil {
		t.Fatalf("failed to create redis kvs: %v", err)
	}
...
func Test_Load(t *testing.T) {
	ctx := context.Background()
	kvs, err := NewRedisKVS(ctx, "127.0.0.1", 6379, "default", "redispw", 300)
	if err != nil {
		t.Fatalf("failed to create redis kvs: %v", err)
	}
