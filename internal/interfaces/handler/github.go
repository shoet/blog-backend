...
func (g *GitHubGetContributionsHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if username == "" {
		response.RespondBadRequest(w, r, errors.New("username is required"))
		return
	}
	fromDateUtc := r.URL.Query().Get("from_date_utc")
	if fromDateUtc == "" {
		response.RespondBadRequest(w, r, errors.New("from_date_utc is required"))
		return
	}
	toDateUtc := r.URL.Query().Get("to_date_utc")
	if toDateUtc == "" {
		response.RespondBadRequest(w, r, errors.New("to_date_utc is required"))
		return
	}

	fromDateTime, err := time.Parse(time.RFC3339, fromDateUtc)
	if err != nil {
		response.RespondBadRequest(w, r, errors.New("from_date_utc is invalid format"))
		return
	}

	toDateTime, err := time.Parse(time.RFC3339, toDateUtc)
	if err != nil {
		response.RespondBadRequest(w, r, errors.New("to_date_utc is invalid format"))
		return
	}
...
func (g *GitHubGetContributionsLatestWeekHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if nubOfLatestWeekStr == "" {
		response.RespondBadRequest(w, r, errors.New("num_of_latest_week is required"))
		return
	}

	numOfLatestWeek, err := strconv.Atoi(nubOfLatestWeekStr)
	if err != nil {
		response.RespondBadRequest(w, r, errors.New("num_of_latest_week is invalid"))
		return
	}
